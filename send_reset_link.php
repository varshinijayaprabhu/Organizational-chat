...
function sendPasswordResetEmail($email, $resetLink) {
    ...
    try {
        ...
        $mail->Password = 'mwekyvsndlneuglr'; // SMTP password
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;

        // Sender and recipient settings
        $mail->setFrom('vivekrkdc2021bca@gmail.com', 'Organizational-chat');
...
if(isset($_POST['email'])) {
    ...
    if($result->num_rows > 0) {
        ...
        if($conn->query($updateTokenQuery) === TRUE) {
            ...
            $resetLink = "http://localhost/Organizational-chat-/reset_password.php?token=$token";
